...
const Index = ({ menu, general, galleries, page }) => {
    ...
    useEffect(() => {
        if (galleries.rows) {
            ...
            let arrays2 = galleries.rows.map(item => item.type === 'video' ? 'youtube' : null)
    ...
    return (
        <Layout pageTitle={`Galeri Foto/Video Kegiataan Sekolah | ${general.organization}`}>
            <Header
                general={general}
                menus={menu?.main}
            />
                            ...
                            {galleries?.rows.map((item, key) => {
                                return (
                                    <div
                                        ...
                                        className="col-lg-4 col-md-6 col-sm-6 portfolio-item filter-app" key={key}>
...
export const getServerSideProps = async ({ query }) => {
    ...
    const res1 = await fetch(`${process.env.API_URL}/public/galleries?page_size=12&page=${page}`)
